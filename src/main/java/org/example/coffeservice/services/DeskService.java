...
public class DeskService {
  ...
  public List<DeskResponseDTO> getAllDesks() {
    try {
      ...
      return desks.stream().map(this::convertToDTO).collect(Collectors.toList());
  ...
  public DeskResponseDTO updateDesk(Long id, DeskRequestDTO deskRequest) {
    try {
      ...
      throw new RuntimeException("Ошибка обновления стола с id " + id, exception);
    }
  }
  ...
  public void deleteDesk(Long id) {
    try {
      ...
      throw new RuntimeException("Ошибка удаления стола с id " + id, exception);
    }
  }
